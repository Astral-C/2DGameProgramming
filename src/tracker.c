...
uint8_t tracker_open_mod(ModTracker* tracker, char* mod){
	...
	fread(name, sizeof(name), 1, mod_file);

	if(strncmp(tracker->module.module_name, name, sizeof(name)) == 0){
		fclose(mod_file);
		return 0; // already loaded mod, keep going
	}

	memset(tracker, 0, sizeof(ModTracker));

	strncpy(tracker->module.module_name, name, sizeof(tracker->module.module_name));
